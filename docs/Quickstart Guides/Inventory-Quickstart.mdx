...
<br /><span class="badge bar badge--success">POST</span> <span class="path-text">/api/v2/inv/inventories/includeExclude</span>
...
1. Campaign Inventory targeting 
...
### Step 3: Campaign Inventory Targeting 
...
Once you choose your desired Inventories, you can include them in a Campaign's targeting strategy. Using the <var>id</var> from the response in Step 2, and a <var>campaignId</var> you can attach an Inventory to the desired Campaign. 
...
### Step 4: Create an Inventory Group
...
[If you have an existing Inventory group and you want to update it, skip to [Step 5](#step-5-update-an-inventory-group)]

The IQM API also allows you to create custom groupings of Inventories with several methods. In this example, we can take the Inventory <var>id</var>s used in Steps 2 and 3 and pass them in the <var>inventoryIds</var> array, along with a <var>groupName</var> and <var>groupTypeId</var> to create a new group.

To review all of the methods, see the [Create a New Inventory Group](/Guidelines/Inventory-API-Guide#create-a-new-inventory-group) endpoint documentation. 
...
### Step 5: Update an Inventory Group 
...
Once you have attached an Inventory ID to a Campaign for targeting and created an Inventory Group with that same Inventory ID, you can check that Inventory <var>groupId</var> with this endpoint. You should see the <var>campaignId</var> in the response. 
