...
Run your first report using IQM's API in this step-by-step quickstart guide.

Get started running your first report with the following endpoints:

<CopyUrl method="POST" url="https://api.iqm.com/api/v3/ua/oauth/token" />
<br /><CopyUrl method="GET" url="https://api.iqm.com/api/v3/master/timezones" />
<br /><CopyUrl method="GET" url="https://api.iqm.com/api/v3/ra/report/dimension-metrics/detail" />
<br /><CopyUrl method="POST" url="https://api.iqm.com/api/v3/ra/report/execute" />

## About IQM reports

IQM's APIs provide access to all dimensions and KPIs of your ad-serving reporting data in JSON format. With the Reports API you can create a customized report with your desired dimensions and metrics.

Reports can be either **daily** or **aggregated**. The daily report includes the date as a dimension and provides the data breakdown by date. Reports provide five top-level dimensions of filtering support, and you can filter on multiple values for each dimension.
...
### More resources
  * [Custom reports<ExternalLink />](https://help.iqm.com/en/collections/11287040-custom-reports) Help Center articles
  * [Report dimensions and metrics<ExternalLink />](https://help.iqm.com/en/collections/11287049-report-dimensions-and-metrics) Help Center articles
  * [Reports API](/guidelines/reports-api) guidelines

## Run a report using the IQM API

This quickstart guide covers how to run a basic IQM report.  
...
Reports are in table format. The metrics comprise the columns, and the dimensions comprise the rows. Necessary information about campaigns is found in the columns.

For more information see this [Help Center<ExternalLink />](https://help.iqm.com/en/collections/11287049-report-dimensions-and-metrics) article collection or [Get dimensions and metrics details](/guidelines/reports-api#get-dimensions-and-metrics-details).
...
For further information see [Get timezones](/guidelines/master-api#get-timezones).
...
function getTimezones(): Promise < Responses > {
  const options = {
    method: 'GET',
    url: 'https://api.iqm.com/api/v3/master/timezones',
  };
...
The report defaults to the previous three months if a start and end date are not included.

All columns are optional, but some must be included to run a report.

At least one dimension is required.

For further information see [Execute a report](/guidelines/reports-api#execute-a-report).
...
| `customerIds` <br /><span class="type-text">array of integers</span> | List of Customer IDs to include in the report, leave empty array [ ] to include all, optional for Workspace users only |
