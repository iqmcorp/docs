...
<tr>
  ...
  <td>Partner</td>
  <td>A user that signs up with the platform and receives authentication credentials: Organization Workspace ID (`owId`) for IQM; Partner ID for The Trade Desk. A Workspace User are also referred to as Advertisers or Customers in different contexts. <p />See [Workspace API](/docs/Guidelines/Workspace-Guidelines) or [IQM Platform Overview](/docs/Getting%20Started/Platform-overview) pages for more details.</td>
</tr>
<tr>
