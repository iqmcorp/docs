...
This tutorial covers how to sign up or invite a new Customer using the IQM API. 
...
Once a Customer has accepted their invitation and the onboarding process is complete other Customer actions can be performed. This tutorial covers a few examples. The rest of the possible Customer actions are documented in the [Workspace API](/guidelines/workspace-api). 
...
Upon invite, Customer status is <var>invited</var>. Once the invitation is accepted, the Customer's status is <var>active</var> and the onboarding process is complete. 
...
A Customer Organization can register itself using the signup API. On signup, the Customer status is <var>pending</var>. This triggers the parent Organization to approve the new Customer signup (see next section). 

Once approved, an invitation email is sent to complete the registration process, and the Customer status updates to <var>active</var>.
...
function AssignCustomerstoUser(): Promise<Responses> {
  const options = {
    method: 'POST',
    url: 'https://api.iqm.com/api/v3/ua/user/customer/assign',
    requestBody: {
      content: {
        "application/json": {
          owIds: `string`,
