...
The minimum requirements are: creating an account with IQM, running at least one campaign, and attaining authentication to use the platform. Review the [Getting Started](/getting-started/before-you-begin) page for details on these requirements before using the IQM API. 

Once these steps are completed see the other [Quickstart Guides](/quickstart-guides/) and the [Guidelines](/guidelines/) pages for a comprehensive overview of the IQM platform's API. 
...
Refer to the [Sign Up and Authenticate](/docs/quickstart-guides/authentication-quickstart-guide.mdx) Quickstart Guide for more information.
...
Many GET endpoints support queries to filter and sort the data (learn more by reviewing [API Filtering and Pagination](/getting-started/api-pagination-guide)). Below are examples of common fields and a response sample for GET resources.
...
In some cases, POST endpoints function like a GET endpoint, where the request schema takes [API Filtering and Pagination](/getting-started/api-pagination-guide) parameters to sort the retrieved data. 
...
The DELETE method allows you to delete resources with the IQM API. DELETE endpoints have the same common response fields as POST endpoints. Below is an example of a DELETE endpoint for a resource. 
...
IQM uses HTTP status codes. Refer to [MDN Documentation<ExternalLink />](https://developer.mozilla.org/en-US/Web/HTTP/Status) on status codes for more information. 

API calls return a JSON formatted response consisting of a <var>success</var> boolean and, often, a <var>message</var>, <var>data</var>, or <var>error</var> property describing the success or failure. Below is a table of common status codes and response samples. 
