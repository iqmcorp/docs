...
Many endpoint will support filtering by some kind of entity ID. The exact parameter will vary based on the entity. For example, the [Get List of Campaigns](/guidelines/campaign-api#get-list-of-campaigns) endpoint supports filtering by <var>conversionId</var>. When a Conversion ID is passed as a query, the endpoint returns all Campaign IDs that are associated with that Conversion at the top of the results. 
