...
The Parameters table below lists the most common filtering and pagination fields used in the IQM API. Their usage varies based on the endpoint. They often have a default value if a value is not specified. The default value is listed as part of an endpoint's documentation if it exists. 

| Parameters |  |
| ---- | --- |
| `ids` <br /><span class="type-text">integer</span> | Filter results by entity ID (varies by endpoint) |
| `searchField` / `keywords` <br /><span class="type-text">string</span> | Filter results by search field |
| `limit` / `noOfEntries` <br /><span class="type-text">integer</span> | Number of entries returned per page |
...
In most cases, filtering the results yields response data with <var>totalRecords</var>, the total number of records for the resource, and <var>filteredRecords</var>, the number of those total records filtered for this set of results. 

### Filtering by ID

Many endpoints support filtering by some kind of entity ID. The exact parameter varies based on the entity. For example, the [Get List of Campaigns](/guidelines/campaign-api#get-list-of-campaigns) endpoint supports filtering by <var>conversionId</var>. When a conversion ID is passed as a query, the endpoint returns all campaign IDs that are associated with that conversion at the top of the results. 
...
In this example the results are filtered with a search keyword. The [Get State Segment](/guidelines/master-api#get-state-segment) endpoint uses the POST method, and therefore its pagination parameters will be passed with a JSON formatted request instead of a query. 
...
The maximum number of entries returned can be specified with the <var>noOfEntries</var> (or sometimes <var>limit</var>) query parameter. Below two entries are retrieved from the [Get List of Inventories](/guidelines/inventory-api#get-list-of-inventories) endpoint. 
...
When the possible number of results exceeds the limit that can be returned, there is a <var>pageNo</var> parameter to specify which page of results is returned. Keeping with the example in the previous section, if a page number of "2" is appended in addition to the two entries, the next two records are retrieved. 
...
Lists of results can often be sorted by a specified parameter. Using the <var>sortBy</var> query in the [Get List of Campaigns](/guidelines/campaign-api#get-list-of-campaigns) endpoint, the returned list of campaigns can be sorted by ascending or descending by appending a <var>+</var> or <var>-</var> symbol to the desired parameter. In this example, the results are sorted by <var>-created</var>: the descending order of creation date. 
